...
If you ever needed to check that dataframes are exactly equals in values, 
you might know it could be unreasonably difficult task.
...
(
    pd.Series([1,2,3, np.nan,5]) == pd.Series([1,2,3, np.nan, 5])
).all()
```
...
Right way to do this is using of [pd.equals(...)](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.equals.html) (don't confuse with `pd.eq(...)`!) or [assert_equal](https://pandas.pydata.org/docs/reference/api/pandas.testing.assert_frame_equal.html#pandas.testing.assert_frame_equal) in older versions of pandas:
